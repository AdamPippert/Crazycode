---
title: SDK
description: Type-safe JS client for crazycode server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The crazycode JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control crazycode programmatically.
...
Create an instance of crazycode:

```javascript
import { createCrazycode } from "@crazycode-ai/sdk"

const { client } = await createCrazycode()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `crazycode.json`, but you can override or add configuration inline:
...
console.log(`Server running at ${crazycode.server.url}`)
...
If you already have a running instance of crazycode, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@crazycode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*crazycode" },
})
