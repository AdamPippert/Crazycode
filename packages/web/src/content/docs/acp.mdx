---
title: ACP Support
description: Use CrazyCode in any ACP-compatible editor.
---

CrazyCode supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use CrazyCode via ACP, configure your editor to run the `crazycode acp` command.

The command starts CrazyCode as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "CrazyCode": {
      "command": "/absolute/path/bin/crazycode",
      "args": ["acp"]
    }
  }
}
...
To open it, use the new 'CrazyCode' agent in the AI Chat agent selector.
...
```lua {6-8}
{
  acp_providers = {
    ["crazycode"] = {
      ...
      args = { "acp" },
      env = {
        CRAZYCODE_API_KEY = os.getenv("CRAZYCODE_API_KEY")
      }
    }
  }
}
...
To use CrazyCode as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use CrazyCode as the ACP agent for chat.
...
CrazyCode works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your CrazyCode config
