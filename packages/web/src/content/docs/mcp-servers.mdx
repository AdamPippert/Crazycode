...
You can add external tools to CrazyCode using the _Model Context Protocol_, or MCP. CrazyCode supports both local and remote servers.
...
You can define MCP servers in your [CrazyCode Config](https://crazycode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="crazycode.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/crazycode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
```jsonc title="crazycode.jsonc" {15}
{
  "$schema": "https://crazycode.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
CrazyCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, CrazyCode will:
...
If the server requires authentication, CrazyCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `crazycode mcp auth <server-name>`.
...
```json title="crazycode.json" {7-11}
{
  "$schema": "https://crazycode.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
crazycode mcp auth my-oauth-server
```
...
```bash
crazycode mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, CrazyCode will store the tokens securely in `~/.local/share/crazycode/mcp-auth.json`.
...
crazycode mcp debug my-oauth-server
```
...
Your MCPs are available as tools in CrazyCode, alongside built-in tools. So you can manage them through the CrazyCode config like any other tool.
...
```json title="crazycode.json" {11, 14-18}
{
  "$schema": "https://crazycode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
crazycode mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect CrazyCode to your Sentry account.
