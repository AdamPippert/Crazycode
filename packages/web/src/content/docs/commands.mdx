...
Create `.crazycode/command/test.md`:

```md title=".crazycode/command/test.md"
---
...
You can add custom commands through the CrazyCode config or by creating markdown files in the `command/` directory.

---

### JSON

Use the `command` option in your CrazyCode [config](/docs/config):

```json title="crazycode.jsonc" {4-12}
{
...
- Global: `~/.config/crazycode/command/`
- Per-project: `.crazycode/command/`

```markdown title="~/.config/crazycode/command/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".crazycode/command/component.md"
---
...
```md title=".crazycode/command/create-file.md"
---
...
```md title=".crazycode/command/analyze-coverage.md"
---
...
```md title=".crazycode/command/review-changes.md"
---
...
```md title=".crazycode/command/review-component.md"
---
...
CrazyCode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
