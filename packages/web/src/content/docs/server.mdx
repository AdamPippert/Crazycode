---
title: Server
description: Interact with crazycode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `crazycode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that a crazycode client can use.
...
```bash
crazycode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
crazycode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
When you run `crazycode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the crazycode server to interact with crazycode programmatically.
:::

This architecture lets crazycode support multiple clients and allows you to interact with crazycode programmatically.

You can run `crazycode serve` to start a standalone server. If you have the
crazycode TUI running, `crazycode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the CrazyCode [IDE](/docs/ide) plugins.
...
The crazycode server exposes the following APIs.
