---
title: Providers
description: Using any LLM provider in CrazyCode.
---

import config from "../../../config.mjs"
export const console = config.console

CrazyCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support for **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your CrazyCode config.
...
in `~/.local/share/crazycode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your CrazyCode
config.
...
CrazyCode Zen is a list of models provided by the CrazyCode team that have been
tested and verified to work well with CrazyCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with CrazyCode Zen.
:::

1. Run the `/connect command, select CrazyCode, and head to [crazycode.ai/auth](https://crazycode.ai/auth).
...
It works like any other provider in CrazyCode. And is completely optional to use
it.
...
To use Amazon Bedrock with CrazyCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running crazycode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY crazycode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile crazycode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX crazycode
   ```
   ...
   For project-specific or persistent configuration, use `crazycode.json`:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for crazycode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX crazycode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX crazycode
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your crazycode config.
...
To use your GitHub Copilot subscription with crazycode:
...
To use Google Vertex AI with CrazyCode:
...
2. Set the required environment variables:
   ...
   Set them while running crazycode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id crazycode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through crazycode, you can always configure it yourself.
...
```jsonc title="~/.config/crazycode/crazycode.jsonc"
{
  "$schema": "https://crazycode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each CrazyCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each CrazyCode conversation listed as a separate session.
...
You can configure crazycode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="crazycode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://crazycode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure crazycode to use local models through LM Studio.

```json title="crazycode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://crazycode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure crazycode to use local models through Ollama.

```json title="crazycode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://crazycode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with CrazyCode:
...
3. Copy the API key for use in CrazyCode.
...
6. **Important**: Before using cloud models in CrazyCode, you must pull the model information locally:
...
CrazyCode Zen is a list of tested and verified models provided by the CrazyCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>CrazyCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **CrazyCode Zen**.

   ```txt
   /connect
   ```

3. Enter your CrazyCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your crazycode config.
...
5. You can also customize them through your crazycode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' crazycode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group crazycode
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with CrazyCode:
...
You can also customize models through your crazycode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with crazycode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in crazycode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `crazycode.json` file in your project directory:

   ```json title="crazycode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://crazycode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="crazycode.json" {9,11,17-20}
{
  "$schema": "https://crazycode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow CrazyCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `crazycode auth list` to see if the credentials
...
2. For custom providers, check the crazycode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your crazycode config.
