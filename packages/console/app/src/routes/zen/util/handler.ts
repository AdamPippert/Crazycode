...
import { and, Database, eq, isNull, lt, or, sql } from "@crazycode-ai/console-core/drizzle/index.js"
import { KeyTable } from "@crazycode-ai/console-core/schema/key.sql.js"
import { BillingTable, SubscriptionTable, UsageTable } from "@crazycode-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@crazycode-ai/console-core/util/price.js"
import { getWeekBounds } from "@crazycode-ai/console-core/util/date.js"
...
import { Black, BlackData } from "@crazycode-ai/console-core/black.js"
...
import { ProviderTable } from "@crazycode-ai/console-core/schema/provider.sql.js"
...
export async function handler(
  ...
  const FREE_WORKSPACES = [
    ...
    "wrk_01K6W1A3VE0KMNVSCQT43BG2SX", // crazycode bench
  ]
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-crazycode-session") ?? ""
    const requestId = input.request.headers.get("x-crazycode-request") ?? ""
    const projectId = input.request.headers.get("x-crazycode-project") ?? ""
    const ocClient = input.request.headers.get("x-crazycode-client") ?? ""
    logger.metric({
      is_tream: isStream,
      session: sessionId,
      request: requestId,
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetch(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-crazycode-request")
          headers.delete("x-crazycode-session")
          headers.delete("x-crazycode-project")
          headers.delete("x-crazycode-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo) {
    ...
    if (!billing.paymentMethodID)
      throw new CreditsError(
        `No payment method. Add a payment method here: https://crazycode.ai/workspace/${authInfo.workspaceID}/billing`,
      )
    if (billing.balance <= 0)
      throw new CreditsError(
        `Insufficient balance. Manage your billing here: https://crazycode.ai/workspace/${authInfo.workspaceID}/billing`,
      )
    ...
    if (
      ...
      throw new MonthlyLimitError(
        `Your workspace has reached its monthly spending limit of $${billing.monthlyLimit}. Manage your limits here: https://crazycode.ai/workspace/${authInfo.workspaceID}/billing`,
      )
    ...
    if (
      ...
      throw new UserLimitError(
        `You have reached your monthly spending limit of $${authInfo.user.monthlyLimit}. Manage your limits here: https://crazycode.ai/workspace/${authInfo.workspaceID}/members`,
      )
