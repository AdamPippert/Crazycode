...
import { createCrazycodeClient, type CrazycodeClient } from "@crazycode-ai/sdk/v2"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: CrazycodeClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createCrazycodeClient({ baseUrl: args.attach })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createCrazycodeClient({ baseUrl: `http://${server.hostname}:${server.port}` })
