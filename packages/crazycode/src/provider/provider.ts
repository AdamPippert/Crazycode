...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async crazycode(input) {
      ...
      const hasKey = await (async () => {
        ...
        if (config.provider?.["crazycode"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from crazycode.json provider config
    ...
    openrouter: async () => {
      return {
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://crazycode.ai/",
            "X-Title": "crazycode",
          },
        },
      }
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "crazycode",
          },
        },
      }
    },
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.CRAZYCODE_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("crazycode")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if crazycode provider is available before using it
    const crazycodeProvider = await state().then((state) => state.providers["crazycode"])
    if (crazycodeProvider && crazycodeProvider.models["gpt-5-nano"]) {
      return getModel("crazycode", "gpt-5-nano")
    }
