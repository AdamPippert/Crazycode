...
import { createCrazycode, type ToolPart } from "@crazycode-ai/sdk"
...
console.log("🚀 Starting crazycode server...")
const crazycode = await createCrazycode({
  port: 0,
})
console.log("✅ Crazycode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await crazycode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new crazycode session...")
    const { client, server } = crazycode
    ...
    console.log("✅ Created crazycode session:", createResult.data.id)
  ...
  console.log("📝 Sending to crazycode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Crazycode response:", JSON.stringify(result, null, 2))
