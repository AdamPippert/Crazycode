...
CrazyCode er også tilgjengelig som en desktop-app. Last ned direkte fra [releases-siden](https://github.com/AdamPippert/Crazycode/releases) eller [crazycode.ai/download](https://crazycode.ai/download).
...
CrazyCode har to innebygde agents du kan bytte mellom med `Tab`-tasten.
...
Les mer om [agents](https://crazycode.ai/docs/agents).

### Dokumentasjon

For mer info om hvordan du konfigurerer CrazyCode, [**se dokumentasjonen**](https://crazycode.ai/docs).

### Bidra

Hvis du vil bidra til CrazyCode, les [contributing docs](./CONTRIBUTING.md) før du sender en pull request.

### Bygge på CrazyCode

Hvis du jobber med et prosjekt som er relatert til CrazyCode og bruker "opencode" som en del av navnet; for eksempel "opencode-dashboard" eller "opencode-mobile", legg inn en merknad i README som presiserer at det ikke er bygget av CrazyCode-teamet og ikke er tilknyttet oss på noen måte.
...
- 100% open source
- Ikke knyttet til en bestemt leverandør. Selv om vi anbefaler modellene vi tilbyr gjennom [CrazyCode Zen](https://crazycode.ai/zen); kan CrazyCode brukes med Claude, OpenAI, Google eller til og med lokale modeller. Etter hvert som modellene utvikler seg vil gapene lukkes og prisene gå ned, så det er viktig å være provider-agnostic.
- LSP-støtte rett ut av boksen
- Fokus på TUI. CrazyCode er bygget av neovim-brukere og skaperne av [terminal.shop](https://terminal.shop); vi kommer til å presse grensene for hva som er mulig i terminalen.
- Klient/server-arkitektur. Dette kan for eksempel la CrazyCode kjøre på maskinen din, mens du styrer den eksternt fra en mobilapp. Det betyr at TUI-frontend'en bare er en av de mulige klientene.
