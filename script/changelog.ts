...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/AdamPippert/Crazycode/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
...
export async function generateChangelog(commits: Commit[], crazycode: Awaited<ReturnType<typeof createCrazycode>>) {
